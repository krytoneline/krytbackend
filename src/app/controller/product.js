...
module.exports = {
  ...
  getProductByslug: async (req, res) => {
    try {
      ...
      let sellerreviews = await Review.find({
    ...
    try {
      ...
      await Promise.all(
        payload.productDetail.map(async (item) => {
          ...
          const commission =
            (Number(item.total) * Number(com.commission)) / 100;
          const finalAmount = Number(item.total) - Number(commission);

          admin.commission =
            (Number(admin.commission) || 0) + Number(commission);
  ...
  createPdf: async (req, res) => {
    try {
      ...
      doc
        .text("Delivery Fee:", 370, totalsY + 20)
      ...
      doc
        ...
        .text(
          "For support, contact us at Rakesh.c@engineer.com",
          50,
          doc.page.height - 85,
          {
            align: "center",
        ...
        .text(
          "Visit us at: https://www.krytonline.com/",
          50,
          doc.page.height - 70,
          {
            align: "center",
  ...
  getrequestProduct: async (req, res) => {
    try {
      ...
      const product = await ProductRequest.find(cond)
        .populate("productDetail.product user")
        .limit(limit * 1)
        .skip((page - 1) * limit)
        .sort({ createdAt: -1 });
